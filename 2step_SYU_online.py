...
# Updated by: Cameron Armstrong (2022)
...
# function that reads in previously collected .csv data for flowrates/temperature     
    def data_reading_online(self,dft_file,df_file,df2_file,df3_file):
    ...
    def Arrays_initialize_online(self):
    ...
    def Arrays_updateIC_online(self):
    ...
    def Temp_offline(self):
        ...
        self.Tj1= self.dft.SMA_temp[len(self.Jkem_raw)-1]+273.15 # temp of first reactor (converting degK to degC)
        ...
        self.df4c = self.df4c.append({'temp':self.Tj1-273.15,'time':self.temp_time[self.j]},ignore_index=True)
    ...
    def Flowrates_offline(self):
        ...
        self.SMA_f = ((self.df2.SMA_fluoro_flow[len(self.fluoro_raw)-1]*0.00387)-0.273)*1.66667*10**-8 # current moving average flowrate flouro (species B) (m3/s)
        self.SMA_a = ((self.df.SMA_acrylate_flow[len(self.acrylate_raw)-1]*0.00897)-.0769)*1.66667*10**-8 # current moving average flowrate acrylate (species A) (m3/s)
        ...
        self.SMA_c = ((self.df3.SMA_cyclo_flow[len(self.cyclo_raw)-1]*0.0269)-2.85)*1.66667*10**-8 # current moving average flowrate flouro (species B) (m3/s)
...
while True:
    ...
    SYU.data_reading_online(dft_file='7-03-temp-CA2-183_correctset.csv',\
                              df_file='acrylate-7-03-CA2-183.csv',\
                              df2_file='fluoro-7-03-CA2-183.csv',\
                              df3_file='cyclo-7-03-CA2-183.csv',)

    SYU.Temp_online() # calls temperature/rate-constant function
    SYU.Arrays_updateIC_online() # calls function to re-initializes arrays
    SYU.Flowrates_online() # calls flowrate function
    SYU.Solver_vector_online() # calls solver 
    SYU.dfR1sol = SYU.dfR1sol.append({'R1_Conc':SYU.C[SYU.nx1-1],'R1_time':SYU.acrylate_time[len(SYU.df2.acrylate_time)-1]},ignore_index=True) # updates model solution array
    ...
    SYU.dfR2sol = SYU.dfR2sol.append({'R2_Conc':SYU.E[SYU.nx2-1]/1000*313.276,'R2_time':SYU.acrylate_time[len(SYU.df2.acrylate_time)-1]},ignore_index=True) # updates model solution array
...
SYU.dfR1sol.to_csv('online_testR1.csv',index=False)    
SYU.dfR2sol.to_csv('online_testR2.csv',index=False) # generates model solution .csv file
